...
import { Sparkles, Mail, User, ArrowRight, Activity } from 'lucide-react';
...
export default function AuthPage() {
  ...
  const { state: { isAuthenticated }, isLoaded, login } = useStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simulate API call
    setTimeout(() => {
      login(email, isLogin ? 'Human Being' : name);
      setLoading(false);
      router.push('/');
    }, 1500);
  };
      ...
      <div className="w-full max-w-[440px] animate-in fade-in slide-in-from-bottom-8 duration-700">
          ...
          <div className="relative">
            <div className="mb-8">
              ...
              <p className="text-sm text-zinc-400 dark:text-zinc-600 mt-1">
                {isLogin ? 'Enter your details to continue.' : 'Create your local profile to start tracking.'}
              </p>
            </div>
        ...
        <div className="mt-12 text-center text-zinc-400 dark:text-zinc-600">
          ...
          <div className="flex flex-wrap justify-center gap-4 text-xs font-medium">
            <span className="flex items-center gap-1.5"><Sparkles size={12} className="text-zinc-300 dark:text-zinc-700" /> No trackings</span>
            <span className="flex items-center gap-1.5"><Activity size={12} className="text-zinc-300 dark:text-zinc-700" /> Local Storage</span>
            <span className="flex items-center gap-1.5"><ArrowRight size={12} className="text-zinc-300 dark:text-zinc-700" /> Fully Private</span>
          </div>
        </div>
      </div>
    </div>
